...
export const Route = createFileRoute("/blog/")({
  head: () => ({
    meta: [
      ...
      { name: "keywords", content: "import from India blog, agricultural export guide, onion import guide, export documents India, FOB CIF pricing" },
    ...
    <main className="min-h-screen bg-white text-slate-900">
      ...
      <section className="py-14 text-white sm:py-20" style={{ background: `linear-gradient(135deg,${NAVY},#06356d)` }}>
        <div className="mx-auto max-w-7xl px-5 sm:px-6">
          ...
          <a href={buildWhatsAppUrl()} target="_blank" rel="noreferrer" className="mt-8 inline-flex items-center gap-2 rounded-full px-6 py-3.5 text-sm font-bold text-white shadow-lg" style={{ background: "#25D366" }}>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {BLOG_POSTS.map((post) => (
            <Link key={post.slug} to="/blog/$slug" params={{ slug: post.slug }} className="group flex min-h-72 flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-[#0057B8] hover:shadow-xl">
              ...
              <div className="mt-4 text-xs font-bold uppercase tracking-widest" style={{ color: BLUE }}>{post.category}</div>
              <h2 className="mt-3 font-display text-xl font-bold leading-snug text-slate-900">{post.title}</h2>
              <p className="mt-3 flex-1 text-sm leading-7 text-slate-600">{post.description}</p>
              <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold" style={{ color: ORANGE }}>
